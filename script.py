...
def get_gemini_response(input_prompt, pdf_content, top_skills):
    model = genai.GenerativeModel('gemini-pro')
    response = model.generate_content([input_prompt, pdf_content, top_skills])
...
def extract_skills_from_resume(file_content, top_skills):
    skills_found = []
    for skill in top_skills.split(','):
        skill = skill.strip()
        skill_pattern = re.compile(re.escape(skill), re.IGNORECASE)
        matches = skill_pattern.findall(file_content)
        
        if matches:
            # Extract relevant projects and years of experience
            relevant_projects = extract_relevant_projects(file_content, skill)
            years_of_experience = extract_years_of_experience(file_content, skill)
            
            skills_found.append({
                "Skill": skill,
                "Match Status": "Yes",
                "Relevant Projects": relevant_projects if relevant_projects else "NA",
                "Years of Experience": years_of_experience if years_of_experience else "NA"
            })
        else:
            skills_found.append({
                "Skill": skill,
...
def extract_relevant_projects(file_content, skill):
    # Placeholder function to extract relevant projects
    # Implement your logic to extract relevant projects based on the skill
    projects = []
    # Example logic (customize as needed)
    for line in file_content.split('\n'):
        if skill.lower() in line.lower() and "project" in line.lower():
            projects.append(line.strip())
    return ", ".join(projects)

def extract_years_of_experience(file_content, skill):
    # Placeholder function to extract years of experience
    # Implement your logic to extract years of experience based on the skill
    experience_pattern = re.compile(r'(\d+)\s+years', re.IGNORECASE)
    matches = experience_pattern.findall(file_content)
    if matches:
...
Objective: Assess the compatibility of the resume with the job description from a Domain Expert perspective. (Eg: Business Analyst(BA), Functional Manager or Project Manager)
Instructions:
